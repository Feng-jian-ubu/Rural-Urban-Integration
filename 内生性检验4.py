...
# 第二阶段回归：用拟合的 DID 作为自变量进行回归，并加入控制变量
control_vars = control_data.columns[2:]  # 控制变量从第三列开始

# 设置自变量：加入预测的 DID 和控制变量
X_second_stage = sm.add_constant(merged_data[['Predicted_DID'] + list(control_vars)])  # 加入常数项和控制变量
